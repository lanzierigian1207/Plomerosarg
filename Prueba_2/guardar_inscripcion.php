...
$profesion = clean_text($payload['profesion'] ?? '', 40);
...
$allowedProfesion = [
    ...
    'arquitecto_ingeniero', 'estudiante_centro_formacion', 'otros'
];
$allowedOrigen = ['facebook', 'instagram', 'casa_sanitarios', 'centro_formacion'];
...
if (!in_array($asociado, $allowedAsociado, true)) {
    ...
    echo json_encode([
        'ok' => false,
        'error' => 'Valor de asociado invalido.'
    ], JSON_UNESCAPED_UNICODE);
    exit;
}

if (!in_array($profesion, $allowedProfesion, true)) {
    http_response_code(422);
    echo json_encode([
        'ok' => false,
...
if ($line === false || file_put_contents($filePath, $line . PHP_EOL, FILE_APPEND | LOCK_EX) === false) {
    ...
    echo json_encode([
        'ok' => false,
        'error' => 'No se pudo guardar la inscripcion.'
    ], JSON_UNESCAPED_UNICODE);
    exit;
}
